...
function processTotalLengthBits(packetValue) {
    ...
    let p = packetValue.slice(15, 15 + maxSubpacketBits);
    let r = [];
    while (p.includes('1')) {
        const [result, newP] = processPacketValue(p);
        r.push(result);
        p = newP;
    }
    return [r, packetValue.slice(15 + maxSubpacketBits), 15 + maxSubpacketBits];
}
...
if (SAMPLE) {
    ...
    ); // 258888628940 - 258,888,628,940 ---- From Reddit
} else {
    const result = processPacket(getInput());
    logAnswer(FIRST, versionValue);

    logAnswer(SECOND, result);
}
